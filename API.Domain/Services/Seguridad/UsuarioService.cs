...
{
    public UsuarioService(IUnitOfWork<Usuario> repositorios, IHttpContextAccessor httpContext) : base(repositorios,
        httpContext)
    {
    }
    ...
    {
        var usuario = await ObtenerPorId(entity.Id) ??
    ...
    {
        var usuario = await ObtenerPorId(usuarioId) ??
                      throw new CustomException
                          { Status = StatusCodes.Status404NotFound, Message = "Elemento no encontrado." };
    ...
    public async Task<Usuario?> ObtenerPorUsername(string username,
        Func<IQueryable<Usuario>, IIncludableQueryable<Usuario, object>>? propiedadesIncluidas = null)
    {
        return await _repositorios.BasicRepository.FirstAsync(entity => entity.Username == username,
            propiedadesIncluidas);
    }

    public async Task<List<Permiso>> ObtenerPermisos(string username)
    {
        return (await _repositorios.Usuarios.FirstAsync(e => e.Username == username,
                query => query.Include(e => e.Rol.RolPermiso).ThenInclude(e => e.Permiso)))?.Rol.RolPermiso
            .Select(e => e.Permiso).ToList() ?? new List<Permiso>();
    }
}