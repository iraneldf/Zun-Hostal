...
public class BasicService<TEntity, TEntityValidator> : IBaseService<TEntity, TEntityValidator>
    where TEntity : EntidadBase where TEntityValidator : AbstractValidator<TEntity>
{
    ...
    public virtual async Task<IDbContextTransaction> IniciarTransaccion()
    {
        return await _repositorios.BasicRepository.StartTransaction();
    }

    public virtual async Task FinalizarTransaccion()
    {
        await _repositorios.BasicRepository.CommitTransaction();
    }

    public virtual async Task RevertirTransaccion()
    {
        await _repositorios.BasicRepository.RollbackTransaction();
    }
    ...
    public virtual async Task<int> SalvarCambios()
    {
        return await _repositorios.BasicRepository.SaveChangesAsync();
    }

    public virtual async Task<EntityEntry<TEntity>> Actualizar(TEntity entity)
    {
        await ValidarAntesActualizar(entity);
        return _repositorios.BasicRepository.Update(EstablecerDatosAuditoria(entity, false));
    }
    ...
    public virtual async Task Actualizar(List<TEntity> entities)
    {
        ...
        _repositorios.BasicRepository.UpdateRange(EstablecerDatosAuditoria(entities, false));
    }

    public virtual async Task<TEntity?> ObtenerPorId(Guid id,
        Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>>? propiedadesIncluidas = null)
    {
        return await _repositorios.BasicRepository.FirstAsync(entity => entity.Id == id, propiedadesIncluidas);
    }

    public virtual async Task<IEnumerable<TEntity>> ObtenerTodos(string? secuenciaOrdenamiento = null,
        Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>>? propiedadesIncluidas = null)
    {
    ...
    public virtual async Task<(IEnumerable<TEntity>, int)> ObtenerListadoPaginado(int cantidadIgnorar,
        int? cantidadMostrar, string? secuenciaOrdenamiento,
        Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>>? propiedadesIncluidas = null,
        params Expression<Func<TEntity, bool>>[] filtros)
    {
        if (cantidadIgnorar < 0)
            throw new CustomException
            {
                Status = StatusCodes.Status404NotFound,
                Message = "La cantidad de elementos a ignorar debe ser mayor o igual a 0."
            };
        if (cantidadMostrar.HasValue && cantidadMostrar.Value <= 0)
            throw new CustomException
            {
                Status = StatusCodes.Status404NotFound,
                Message = "La cantidad de elementos a mostrar debe ser mayor a 0."
            };
    ...
    public virtual async Task ValidarAntesCrear(TEntity entity)
    {
        ...
        var objectType = Type.GetType(typeof(TEntityValidator).AssemblyQualifiedName ?? string.Empty);
        await (Activator.CreateInstance(objectType, _repositorios) as AbstractValidator<TEntity>)
    ...
    private IOrderedQueryable<TEntity> OrdenarLista(IQueryable<TEntity> query, string? secuenciaOrdenamiento)
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(secuenciaOrdenamiento))
            {
                ...
                foreach (var item in secuenciaOrdenamiento.Split(',', StringSplitOptions.RemoveEmptyEntries))
                    dictionarioOrdenamiento.Add(item.Split(':', StringSplitOptions.RemoveEmptyEntries)[0],
                        item.Split(':', StringSplitOptions.RemoveEmptyEntries)[1]);
                ...
                if (dictionarioOrdenamiento.Any())
                {
                    if (dictionarioOrdenamiento.First().Value == "asc")
                        ...
                        throw new CustomException
                        {
                            ...
                            Message = "Error al ordenar; verifique la secuencia de ordenamiento."
                        };


                    for (var i = 1; i < dictionarioOrdenamiento.Count;)
                    {
                        if (dictionarioOrdenamiento.ElementAt(i).Value == "asc")
                            ...
                            queryOrdenada =
                                queryOrdenada.ThenByColumnDescending(dictionarioOrdenamiento.ElementAt(i).Key);
                        else
                            throw new CustomException
                            {
                                Status = StatusCodes.Status400BadRequest,
        ...
        catch (Exception)
        {
            ...
            {
                Status = StatusCodes.Status400BadRequest,
                Message = "Error al ordenar; verifique la secuenncia de ordenamiento."
            };
        }
    }
}