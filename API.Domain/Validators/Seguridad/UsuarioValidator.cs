...
public class UsuarioValidator : AbstractValidator<Usuario>
{
    ...
    public UsuarioValidator(IUnitOfWork<Usuario> repositorios)
    {
        ...
        RuleFor(m => m).MustAsync(async (usuario, cancelacion) => !await _repositorios.Usuarios.AnyAsync(e =>
                e.Id != usuario.Id && e.Nombre == usuario.Nombre && e.Apellidos == usuario.Apellidos))
            .OverridePropertyName(nameof(Usuario.NombreCompleto))
            .WithMessage("Ya existe un usuario con este mismo nombre completo.");

        RuleFor(m => m).MustAsync(async (usuario, cancelacion) =>
                !await _repositorios.Usuarios.AnyAsync(e => e.Id != usuario.Id && e.Username == usuario.Username))
