...
public interface IBaseService<TEntity, TEntityValidator> where TEntity : EntidadBase
    where TEntityValidator : AbstractValidator<TEntity>
{
    ...
    Task<TEntity?> ObtenerPorId(Guid id,
    ...
    Task<(IEnumerable<TEntity>, int)> ObtenerListadoPaginado(int cantidadIgnorar, int? cantidadMostrar,
    ...
    /// </summary>
    Task<IEnumerable<TEntity>> ObtenerTodos(string? secuenciaOrdenamiento = null,
        Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>>? propiedadesIncluidas = null);
