...
public class PermisoController : Controller
{
    ...
    public virtual async Task<IActionResult> ObtenerSelectList([FromQuery] ObtenerSelectListInputDto inputDto)
    {
        inputDto.NombreCampoTexto =
            typeof(Permiso).GetProperties().FirstOrDefault(e => e.Name.ToLower() == inputDto.NombreCampoTexto.ToLower())
                ?.Name ?? string.Empty;
        inputDto.NombreCampoValor =
            typeof(Permiso).GetProperties().FirstOrDefault(e => e.Name.ToLower() == inputDto.NombreCampoValor.ToLower())
                ?.Name ?? string.Empty;
