...
public class RolController : BasicController<Rol, RolValidator, DetallesRolDto, CrearRolInputDto, ActualizarRolInputDto,
    ListadoPaginadoRolDto, FiltrarConfigurarListadoPaginadoRolIntputDto>
{
    private readonly IRolPermisoService _rolPermisoService;

    public RolController(IMapper mapper, IRolService servicioRol, IRolPermisoService rolPermisoService,
        IHttpContextAccessor httpContext) : base(mapper, servicioRol, httpContext)
    {
    ...
    {
        return await base.ObtenerSelectList(inputDto);
    }
    ...
    protected override Task<(IEnumerable<Rol>, int)> AplicarFiltrosIncluirPropiedades(
        FiltrarConfigurarListadoPaginadoRolIntputDto inputDto)
    {
        //agregando filtros
    ...
    protected override async Task<Rol?> ObtenerElementoPorId(Guid id)
    {
        return await _servicioBase.ObtenerPorId(id,
            query => query.Include(e => e.RolPermiso).ThenInclude(e => e.Permiso));
    }

    protected override async Task<IEnumerable<DetallesRolDto>> ObtenerTodosElementos(
        string? secuenciaOrdenamiento = null)
    {
        return _mapper.Map<IEnumerable<DetallesRolDto>>(await _servicioBase.ObtenerTodos(secuenciaOrdenamiento,
            query => query.Include(e => e.RolPermiso).ThenInclude(e => e.Permiso)));
    }
}