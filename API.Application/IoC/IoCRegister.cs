...
using API.Domain.Services.Hostal;
...
public static class IoCRegister
{
    ...
    {
        services.AddControllers().ConfigureApiBehaviorOptions(_ =>
        {
    ...
    {
        var databaseType = configuration["DatabaseType"] ?? "MSSqlServer";
        ...
        switch (databaseType)
        {
            case "MSSqlServer":
                ...
                services.AddDbContext<TrazasDbContext>(options =>
                    options.UseSqlServer(configuration.GetConnectionString("TrazasContext")));
                break;
            case "MySqlServer":
                services.AddDbContext<ApiDbContext>(options =>
                    options.UseMySql(configuration.GetConnectionString("APIContext"),
                        ServerVersion.AutoDetect(configuration.GetConnectionString("APIContext"))));
                services.AddDbContext<TrazasDbContext>(options =>
                    options.UseMySql(configuration.GetConnectionString("TrazasContext"),
                        ServerVersion.AutoDetect(configuration.GetConnectionString("TrazasContext"))));
                break;
        }
    ...
    public static IServiceCollection RegistrarServiciosDominio(this IServiceCollection services)
    {
        ...
        services.AddScoped<IAmaDeLlaveService, AmaDeLlaveService>();
        services.AddScoped<IAmaDeLlaveHabitacionService, AmaDeLlaveHabitacionService>();

        services.AddScoped(typeof(IBaseService<EntidadBase, AbstractValidator<EntidadBase>>),
            typeof(BasicService<EntidadBase, AbstractValidator<EntidadBase>>));
